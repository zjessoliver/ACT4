...
namespace ACT4
{
    public partial class Form1 : Form
    {
        ...
        {
            if (getAttackingPairs(currentState) <= 0 || temperature <= 0)
                return;
            simulated_annealing();
        }
        public void simulated_annealing()
        {
            ...
            Point randomMove = (Point)chooseMove(bMoves);

            SixState newState = new SixState(currentState);

            newState.Y[randomMove.X] = randomMove.Y;

            int deltaE = getAttackingPairs(newState) - getAttackingPairs(currentState);
